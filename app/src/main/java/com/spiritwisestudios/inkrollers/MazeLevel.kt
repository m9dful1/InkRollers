...
class MazeLevel(
    ...
    private fun generateMaze() {
        ...
        visited[cy][cx] = true
        var visitedCells = 1
        val rnd = Random(seed) // Use the seed passed in constructor

        while (visitedCells < cellsX * cellsY) {
            ...
            if (nbrs.isNotEmpty()) {
                ...
                when (dir) {
                    0 -> horizontalWalls[cy][cx] = false          // remove north wall of current
                    1 -> horizontalWalls[ny][cx] = false          // remove north wall of next (current south)
                    2 -> verticalWalls[cy][nx] = false            // remove west wall of next
                    3 -> verticalWalls[cy][cx] = false            // remove west wall of current
                }
        ...
        // carve entrance (north of ENTRANCE) and exit (south of EXIT)
        horizontalWalls[ENTRANCE.second][ENTRANCE.first] = false
        horizontalWalls[EXIT.second + 1][EXIT.first] = false
    }
